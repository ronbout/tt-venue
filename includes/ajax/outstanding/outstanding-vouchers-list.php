...
$order_rows = $wpdb->get_results($wpdb->prepare("
		SELECT p.post_title,
			...
			i.order_id, i.order_item_id as item_id, i.downloaded as downloaded,i.paid as paid,
...
$payable = display_orders_table($order_rows, $expired_val, $product_price, $vat_val, $commission_val );
?>
<p class="pimage">
...
function display_order_table_row($order_item_info, $expired_val) {
	...
	<tr data-order-id="<?php echo $order_item_info->order_id ?>" 
			...
			data-order-item-id="<?php echo $order_item_info->item_id ?>"
	>
		...
		<td id="td-btn-order-id-<?php echo $order_item_info->order_id ?>" class="text-center">
					<span><?php echo explode(' ',$order_item_info->order_date)[0] ?></span>
		</td> 
	</tr>
	<?php
}
