...
profile_photo.addEventListener('mouseenter', e => {
    ...
    update_profile_photo.setAttribute('class', 'btn btn-primary animate__animated animate__bounceInDown');
});
...
input_file.addEventListener('change', e => {
    ...
    if(file.length > 0){
        ...
        if(/localhost/.test(window.location.href)){
            xhttp.open("POST", url+"/taste/wp-content/plugins/thetaste-venue/includes/ajax/profile_pic_upload.php", true);
            xhttp.onreadystatechange = function () {
                if(this.readyState === 4 && this.status === 200){
                    ...
                    if(response.includes(sub_err)) {
                        ...
                        img.setAttribute('src', `${url}/taste/wp-content/plugins/thetaste-venue/includes/ajax/photos/${response}`);
                        update_profile_photo.style.display = "none";
                    }
                }
            };
